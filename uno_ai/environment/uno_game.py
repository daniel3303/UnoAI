...
class UNOGame:
    ...
    def play_card(self, player: int, card_index: int, chosen_color: Optional[CardColor] = None) -> Dict[str, Any]:
        ...
        if played_card.type in [CardType.WILD, CardType.WILD_DRAW_FOUR]:
            if chosen_color and chosen_color != CardColor.WILD:
                ...
                # Default to red if no color chosen
    ...
    def draw_card(self, player: int) -> Dict[str, Any]:
        ...
        if len(self.deck) == 0:
            # If still no cards after reshuffle, force end turn
            self._next_player()
        ...
        if top_card and card.can_play_on(top_card):
            # Player CAN play the drawn card - they must decide
            return {
                "valid": True,
                "can_play_drawn": True,
            ...
            # Player CANNOT play the drawn card - turn ends automatically
            self._next_player()
            return {
                "valid": True,
    ...
    def _handle_card_effects(self, card: Card) -> Dict[str, Any]:
        ...
        if card.type == CardType.SKIP:
            ...
            self._force_draw(next_player, 2)
            self._next_player()
            result["skip_next"] = True

        elif card.type == CardType.WILD_DRAW_FOUR:
            next_player = (self.current_player + self.direction) % self.num_players
            self._force_draw(next_player, 4)
            self._next_player()
            result["skip_next"] = True
